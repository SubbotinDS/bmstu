...
W2 = 1 / (a * s^2 + b2 * s + 1);  % система с увеличенным демпфированием (ξ = 0.42)
...
legend(sprintf('ξ = %.2f', xi1), sprintf('ξ = %.2f', xi2), 'Location', 'best');
hold off;
...
denominator = sqrt((T1 * w).^2 + 1) .* sqrt((1 - T2_sq * w.^2).^2 + (damping * w).^2);
A = numerator ./ denominator;
H = 20 * log10(A);

% Фаза (с устранением разрывов)
theta_rad = atan(T2 * w) - atan(T1 * w) - atan2(damping * w, 1 - T2_sq * w.^2);
...
s = tf('s');
W_tf = K * (T2 * s + 1) / ((T1 * s + 1) * (T2_sq * s^2 + damping * s + 1));
...
xline(1/T2, 'm--', 'DisplayName', sprintf('ω₂ = 1/T₂ = %.2f', 1/T2));
...
% Создание передаточных функций
s = tf('s');
W_open = K * (T2 * s + 1) / ((T1 * s + 1) * (T2_sq * s^2 + damping * s + 1));  % разомкнутая
